...
class UserController {
    ...
    async createUsers(req, res) {
        var _a;
        const { name, email, profile, password, email_verificad, passwordResetToken, passwordResetExpires, emailResetToken, emailResetExpires, phone, phone_verificad, } = req.body;
        ...
        try {
            ...
            await userRepository_1.userRepository.find({
                relations: {
                    profile: true,
                    projects: true,
                },
            });
            const token = (0, jsonwebtoken_1.sign)({ id: newUser.id }, (_a = process.env.JWT_PASS) !== null && _a !== void 0 ? _a : "G@zao5107", { expiresIn: "8h" });
    ...
    async login(req, res) {
        ...
        try {
            ...
            const token = (0, jsonwebtoken_1.sign)({ id: user.id }, (_a = process.env.JWT_PASS) !== null && _a !== void 0 ? _a : "G@zao5107", {
                expiresIn: "8h",
            });
    ...
    async getAllUsers(req, res) {
        const response = await userRepository_1.userRepository.find({
            relations: {
                profile: true,
                projects: {
                    status: true,
    ...
    async changeUserInfo(req, res) {
        try {
            ...
            const userUpdate = await userRepository_1.userRepository.findOneBy({
                id: req.user.id,
            });
            const userExists = await userRepository_1.userRepository.findOneBy({ email });
            if (userExists && userExists.id !== (userUpdate === null || userUpdate === void 0 ? void 0 : userUpdate.id)) {
                return res
                    .status(400)
            ...
            if (userUpdate) {
                ...
                userRepository_1.userRepository.update(userUpdate.id, {
    ...
    async verificPass(req, res) {
        try {
            const { password } = req.body;
            const userLogged = await userRepository_1.userRepository.findOneBy({
                id: req.user.id,
            });
            if (userLogged) {
                const verifyPass = await bcrypt_1.default.compare(password, userLogged === null || userLogged === void 0 ? void 0 : userLogged.password);
                if (!verifyPass) {
    ...
    async emailVerificad(req, res) {
        ...
        try {
            ...
            if (!userLogged) {
                ...
                userRepository_1.userRepository.update(userLogged.id, {
